...
public:
  ...
  void OnRecv(std::function<void(std::shared_ptr<Connection>)> fn);

  void CheckConnectionTimeout(int fd, EventLoop *sub_loop);
...
private:
  ...
  std::unordered_map<int, std::shared_ptr<Connection>> connections_; // TCP连接
  std::unique_ptr<ThreadPool> thread_pool_; // 线程池
  std::function<void(std::shared_ptr<Connection>)> on_connect_;
  std::function<void(std::shared_ptr<Connection>)> on_recv_;
};
