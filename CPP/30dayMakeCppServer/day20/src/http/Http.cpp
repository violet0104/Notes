...
bool HttpParser::Parse(Buffer &buffer) {
  ...
  while (buffer.HasData() && state_ != HttpParseState::Finish &&
    ...
    case HttpParseState::RequestLine: {
      // 从 Buffer 中尝试提取一行（以 \r\n 为分隔符）
      auto line = buffer.FindAndPopUntil("\r\n");
      if (!line.has_value()) {
...
bool HttpParser::ParseRequestLine(const std::string &line) {
  ...
  if (!(iss >> method_str >> path >> version)) {
    printf("HttpParser: 解析请求行失败: %s\n", line.c_str());
    return false;
  }

  if (!request_.SetMethod(method_str)) {
    printf("HttpParser: 未知的 HTTP 方法: %s\n", method_str.c_str());
    return false;
  }
  ...
  if (!request_.SetVersion(version)) {
    printf("HttpParser: 未知的 HTTP 版本: %s\n", version.c_str());
    return false;
  }
  return true;
}
...
bool HttpParser::ParseHeader(const std::string &line) {
  ...
  if (pos == std::string::npos) {
    ...
    pos = line.find(':');
    if (pos == std::string::npos) {
      printf("HttpParser: 解析 Header 失败: %s\n", line.c_str());
      return false;
    }
...
HttpServer::HttpServer(const char *ip, uint16_t port) {
  http_server_ = std::make_unique<TcpServer>(ip, port);
}

HttpServer::~HttpServer() {}

void HttpServer::Start() {
  http_server_->OnRecv([this](Connection *conn) {
    ...
    if (parser->isFinished()) {
      ...
      // 单次 HTTP 请求处理完毕。情况上下文以支持后续可能的 Keep-Alive 复用该
      // TCP 请求
      parser->Reset();
    }
  });
  printf("Http Server starts running! Please visit "
         "http://127.0.0.1:1234/ in your browser.\n");
  http_server_->Start();
}
