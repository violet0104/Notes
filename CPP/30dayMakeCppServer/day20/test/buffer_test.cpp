...
void TestFindAndPopUntil_StickyPacket() {
  ...
  assert(res1.value() == "GET / HTTP/1.1");

  auto res2 = buf.FindAndPopUntil("\r\n");
  assert(res2.has_value());
  assert(res2.value() == "Host: localhost");
...
void TestFindAndPopUntil_HalfPacket() {
  ...
  auto res1 = buf.FindAndPopUntil("\r\n");
  assert(!res1.has_value()); // Should not find the delimiter yet
  ...
  auto res2 = buf.FindAndPopUntil("\r\n");
  assert(res2.has_value());
  assert(res2.value() == "GET / HTTP/1.1");
  assert(!buf.HasData());
...
void TestRetrieve() {
  ...
  std::optional<std::string> res = buf.FindAndPopUntil("0");
  assert(res.has_value());
  assert(res.value() == "456789");
